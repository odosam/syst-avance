...
void copy(char *bin_input_param, char *bin_output_param)
{
  int f1 = open(bin_input_param, O_RDONLY);
  if (f1 < 0)
  {
    perror("erreur");
    exit(EXIT_FAILURE);
  }
  int f2 = open(bin_output_param, O_WRONLY | O_CREAT | O_TRUNC, 0644);
  if (f2 < 0)
  {
    perror("erreur");
    exit(EXIT_FAILURE);
  ...
  int read_len;
  while ((read_len = read(f1, buffer, BUFFER_SIZE)) != 0)
  {
    write(f2, buffer, read_len);
  }
  close(f1);
  close(f2);
}
